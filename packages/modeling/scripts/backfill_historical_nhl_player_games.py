...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill historical NHL player-game rows for modeling")
    parser.add_argument("--data-root", default="packages/modeling/data", help="Root data directory")
    parser.add_argument("--games-csv", help="Override path to normalized historical games CSV")
    ...
    games_csv_path = Path(args.games_csv) if args.games_csv else data_root / "processed" / "historical_games" / "nhl_games.csv"
    output_csv_path = data_root / "processed" / "historical_player_games" / "nhl_player_games.csv"
    raw_snapshot_root = data_root / "raw" / "nhl_gamecenter"
    ...
    result = ingest_historical_player_games(
